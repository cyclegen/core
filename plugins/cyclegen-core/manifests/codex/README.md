...
| **ブロック機構** | `exit 2` + stderr でブロック可（CCと同一）。★**ただしモデルが直接ツールを呼んだ経路にかぎる** | ✅**CV6実機PASS**: `check-cycle-complete.sh` が Codex でも exit 2 でブロック（stdin JSONは `.tool_input.<arg>` でキー互換）。★**この実測は Codex CLI 0.142.5 のもの**。🔴**CYCLE17.6.6／F-33**: Codex Desktop（`cli_version 0.148.0-alpha.9`）の **code mode**（`exec` の中から `ALL_TOOLS` 経由でMCPを呼ぶ経路・`call_id` が `exec-` で始まる）では、**PreToolUse hook が掛からない**（`cycle_id="CYCLE99"`・ダイジェスト不在のまま `cycle_complete` が成功したことをホスト側 rollout log で実測）。**hookは壊れていない。迂回されている。**★**この経路は例外ではなく日常**（ツール列挙も同じ経路を通る）。ただしファイル存在ガードは捏造突破可＝真ゲートはHITL（finding#4）。→ ★**根治はサーバ側検証（FR066 P0）** |
